	...
	*particles : tuple of :obj:`str`
	...
	if particles is not None:
		...
		for particle in particles:
			...
			#ret[:,:] += np.asarray(np.vectorize(PBH_primary_spectrum, excluded = ['spin']).__call__( energy=energy[:,None], PBH_mass=mass[None,:], spin=spin, **DarkOptions))
	else:
...
def PBH_mass_at_z(initial_PBH_mass, redshift=None, **DarkOptions):
	...
	temp_t = 10**np.linspace(np.log10(time_at_z(z_start)), np.log10(time_at_z(1.)), int(1e5))
