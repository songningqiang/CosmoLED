...
int thermodynamics_init(
  ...
  if(pth->annihilation >0 || pth->decay_fraction > 0 || pth->PBH_accreting_mass > 0 || pth->PBH_evaporating_mass > 0){
    ...
    if(pth->has_on_the_spot==_FALSE_ && pth->energy_repart_coefficient!=no_factorization){
      ...
      if (pth->energy_deposition_function == DarkAges || pth->energy_deposition_function == DarkAges_LED) {
...
double kn(int n){
	...
	return pow(8.*pow(M_PI, (-(n+1.)/2.))*exp(lgamma((n+3.)/2.))/(n+2.), (1./(n+1.)));
}
double get_radius_from_mass(double mass, int n, double Mstar){
	/*Returns the radius of a black hole of a given mass
	...
	Returns
	-------
	Radius of the black hole (*in units of* :math:`\\mathrm{GeV^-1}`)
	*/
	double M = mass*gramtoGeV;
	return kn(n)/Mstar*pow(M/Mstar, (1./(n+1.)));
}
double get_temperature_from_mass(double mass, int n, double Mstar){
	/*Returns the temperature of a black hole of a given mass
	...
	*/
	return (n+1.)/(4.*M_PI*get_radius_from_mass(mass,n,Mstar));
}
double QCD_factor(double T_BH, double sigmoid_factor){
	double Lam_QCD = 0.3;
	double sigma = 0.1;
	return 1. / (1. + exp(- sigmoid_factor*(log10(T_BH) - log10(Lam_QCD))/sigma));
}
double PBH_dMdt(double PBH_mass, double time, double scale, int n, double Mstar){
	...
	this method is not using the variable :code:`time`
	...
	*/
	...
	if (PBH_mass > 0){
		double ret = 0.;
		double rh = get_radius_from_mass(PBH_mass, n, Mstar);
		double TH = (n+1.)/(4.*M_PI*rh);
		for (int i=0; i<17; i++){
			double pmass = particle_dict[i][0];
			double spin = particle_dict[i][1];
			double dof = particle_dict[i][2];
			double sigmoid_factor = particle_dict[i][3];
			double xi, b, c;
			if (spin == 0.5){ dof*=2.; }
			if (spin == 0.){
				xi = xi_scalar[n];
				...
				char errmsg[1024];
				class_stop(errmsg,"incomprehensible input '%f' for the particle spin",spin);
			}
			if (sigmoid_factor != 0.){ret += dof*xi*exp(-b*pow(pmass/TH,c))*QCD_factor(TH,sigmoid_factor);}
			else {ret += dof*xi*exp(-b*pow(pmass/TH,c));}
		}
		
		//add gravitons
		ret += xi_graviton[n];
		ret = ret/(2.*M_PI)/(rh*rh);
...
int PBH_evaporating_mass_time_evolution(
        ...
        for(i_step = 0; i_step < preco->PBH_table_size; i_step++) {
          ...
          */
		  ...
		  if(preco->energy_deposition_function == DarkAges){	
		  ...
		  else if(preco->energy_deposition_function == DarkAges_LED){
			f = PBH_dMdt(current_mass, 0., 1., preco->PBH_n, preco->PBH_Mpl);
		  }
          class_call(background_tau_of_z(pba,
                 loop_z,
                 &loop_tau),
          ...
          if (i_step > 0) {
            if (current_mass > 0.5*preco->PBH_evaporating_mass) {
              //current_mass = current_mass - 5.34e-5*f*pow(current_mass/1e10,-2)*1e10 * dt;
				current_mass = current_mass - f * dt;
            }
            else {
...
int thermodynamics_energy_injection(
  ...
  if (preco->annihilation > 0 || preco->decay_fraction > 0 || preco->PBH_accreting_mass > 0 || preco->PBH_evaporating_mass > 0 ) {
    ...
    if (preco->has_on_the_spot == _FALSE_) {
      ...
      // // /***********************************************************************************************************************/
      else if(preco->energy_deposition_function == DarkAges || preco->energy_deposition_function == DarkAges_LED){
   ...
   /** - Compute the recombination history by calling hyrec_compute.
         No CLASS-like error management here, but YAH working on it :) **/
          ...
          //  hyrec_data.cosmo->inj_params->nz = (long) floor(2+log((1.+hyrec_data.cosmo->inj_params->zstart)/(1.+hyrec_data.cosmo->inj_params->zend))/hyrec_data.cosmo->inj_params->dlna);
           ...
           else if(pth->energy_deposition_function==function_from_file || pth->energy_deposition_function==DarkAges || pth->energy_deposition_function==DarkAges_LED) {
...
int fill_recombination_structure(struct precision * ppr,
 ...
 class_test((ppr->recfast_Hswitch != _TRUE_) && (ppr->recfast_Hswitch != _FALSE_),
  ...
  /* parameters for large extra dimensions */
  preco->PBH_n = pth->PBH_n;
  preco->PBH_Mpl = pth->PBH_Mpl;
