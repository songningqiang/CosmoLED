...
int input_read_parameters(
  ...
  */
  ...
  class_read_int("Number_extra_dimensions",pth->PBH_n);
  class_test(pth->PBH_n != 0 && pth->PBH_n != 1  && pth->PBH_n != 2 && pth->PBH_n != 3 && pth->PBH_n != 4 && pth->PBH_n != 5 && pth->PBH_n != 6,errmsg,
			 "you have specified a number of dimension that is not in the list. Please choose a number from 0 to 6.\n");
  class_call(parser_read_string(pfc,"Bulk_Planck_scale",&string1,&flag1,errmsg),
			 errmsg,
			 errmsg);  
  if (flag1 == _TRUE_) {
	  if ((strstr(string1,"Mpl") != NULL) || (strstr(string1,"mpl") != NULL)) {
		  pth->PBH_Mpl = 1.22e19;
	  }
	  else {
		  pth->PBH_Mpl = atof(string1);
		  if (!(pth->PBH_Mpl > 0.)) {
			  class_stop(errmsg,"incomprehensible input '%s' for the field 'Bulk_Planck_scale'",string1);
		  }
	  }
  }  

  
  //added to test log prior
  class_call(parser_read_string(pfc,
                                "log10_PBH_fraction",
                                &(string1),
                                &(flag1),
  ...
  if (flag1 == _TRUE_) {
    if ((strstr(string1,"y") != NULL) || (strstr(string1,"Y") != NULL)) {
      pth->PBH_fraction = pow(10., pth->PBH_fraction);
    }
    else {
      if ((strstr(string1,"n") != NULL) || (strstr(string1,"N") != NULL)) {
        pth->PBH_fraction = pth->PBH_fraction;
      }
      else {
        class_stop(errmsg,"incomprehensible input '%s' for the field 'log10_PBH_fraction'",string1);
      }
    }
  }  
  ...
  if (flag2 == _TRUE_){
    ...
    if (strcmp(string1,"DarkAges_f_eff") == 0) {
      ...
      pth->has_on_the_spot = _FALSE_;
      flag3=_TRUE_;
  }    if (strcmp(string1,"DarkAges_LED") == 0) {
      pth->energy_deposition_function=DarkAges_LED;
      pth->energy_repart_coefficient = no_factorization;
      pth->has_on_the_spot = _FALSE_;
      flag3=_TRUE_;
  }
  if (strcmp(string1,"DarkAges_LED_f_eff") == 0) {
      pth->energy_deposition_function=DarkAges_LED;
      pth->has_on_the_spot = _FALSE_;
      flag3=_TRUE_;
  }
    ...
    class_test(flag3==_FALSE_, errmsg,
      "Could not identify energy_deposition_function, check that it is one of 'Analytical_approximation', 'DarkAges','DarkAges_f_eff', 'DarkAges_LED','DarkAges_LED_f_eff','from_file','No_deposition'.");
  }
  ...
  if(pth->energy_deposition_function==DarkAges_LED){
	  class_call(parser_read_string(pfc,
									"print_energy_deposition_function",
									&(string1),
									&(flag1),
		  ...
		  */
		  ...
		  if (strcmp(string1,"built_in") == 0) {
			  ...
			  if(pth->PBH_evaporating_mass > 0){
				  strcat(ppr->command_fz,"/DarkAgesModule/bin/DarkAges_LED --hist=evaporating_PBH --mass=");
				  sprintf(string2,"%g",pth->PBH_evaporating_mass);
				  strcat(ppr->command_fz,string2);
				  strcat(ppr->command_fz," --dims=");
				  sprintf(string2,"%d",pth->PBH_n);
				  strcat(ppr->command_fz,string2);
				  strcat(ppr->command_fz," --mpl=");
				  sprintf(string2,"%g",pth->PBH_Mpl);
				  strcat(ppr->command_fz,string2);				  
			  }
