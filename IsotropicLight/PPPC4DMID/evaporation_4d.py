...
def PBH_mass_at_t_ode(initial_PBH_mass, t_end, **DarkOptions):
	u"""Solves the ODE for the PBH mass (:meth:`PBH_dMdt <DarkAges.evaporator.PBH_dMdt>`)
	and returns the masses at the redshifts given by the input :code:`redshift`
	...
	# Workaround for dealing with **DarkOptions inside the ODE-solver.
	ODE_to_solve = lambda t,m: PBH_dMdt(m, t, scale=scale, **DarkOptions)
	jac_to_use = lambda t,m: jac(m,t, scale=scale, **DarkOptions)    

	#vode turns out to be the best
	solver = ode(ODE_to_solve, jac = jac_to_use).set_integrator('vode', nsteps = 1, first_step = 1.e-40, method = "BDF", rtol = 1.e-25, atol = 1e-15, order = 5)
...
def Eqmean(initial_PBH_mass):
	...
	Mth = get_mass_from_temperature(Lam_QCD)
	#print(Mth)

	#Mf > 0 to avoid divergence at 0
	Mf = 1e-10
	if initial_PBH_mass > Mth:
		numerator = (quad(lambda x: np.heaviside(get_temperature_from_mass(x)-Lam_QCD, 1)*get_temperature_from_mass(x)*x, Mth, Mf))[0]
	else:
		numerator = (quad(lambda x: np.heaviside(get_temperature_from_mass(x)-Lam_QCD, 1)*get_temperature_from_mass(x)*x, initial_PBH_mass, Mf))[0]
	if initial_PBH_mass > Mth:
		denominator = (quad(lambda x: np.heaviside(get_temperature_from_mass(x)-Lam_QCD, 1)*x, Mth, Mf))[0]
	else:
		denominator = (quad(lambda x: np.heaviside(get_temperature_from_mass(x)-Lam_QCD, 1)*x, initial_PBH_mass, Mf))[0]
	Ti = get_temperature_from_mass(initial_PBH_mass)
	return numerator/denominator/Ti

def Eqmeanexact(initial_PBH_mass):
	u"""find <E_q> in Eq. 10 of 2006.03608
	initial_PBH_mass: initial BH mass in grams M_i
	Retures: <E_q>/T_i
	"""
	...
	scale  = 1
	if initial_PBH_mass > Mth:
		numerator = (quad(lambda x: np.heaviside(get_temperature_from_mass(x)-Lam_QCD, 1)*get_temperature_from_mass(x)**2/PBH_dMdt(x, 0, scale=scale), Mth, Mf))[0]
	else:
		numerator = (quad(lambda x: np.heaviside(get_temperature_from_mass(x)-Lam_QCD, 1)*get_temperature_from_mass(x)**2/PBH_dMdt(x, 0, scale=scale), initial_PBH_mass, Mf))[0]
	if initial_PBH_mass > Mth:
		denominator = (quad(lambda x: np.heaviside(get_temperature_from_mass(x)-Lam_QCD, 1)*get_temperature_from_mass(x)/PBH_dMdt(x, 0, scale=scale), Mth, Mf))[0]
	else:
		denominator = (quad(lambda x: np.heaviside(get_temperature_from_mass(x)-Lam_QCD, 1)*get_temperature_from_mass(x)/PBH_dMdt(x, 0, scale=scale), initial_PBH_mass, Mf))[0]
	Ti = get_temperature_from_mass(initial_PBH_mass)
	return numerator/denominator/Ti

print("{:e}".format(get_mass_from_temperature(0.3)))

fEqmean = 4.2292
fhadronmean = 3.6735

Ms = np.logspace(8, 14, 1000)
#Eqs = np.asarray(np.vectorize(Eqmean).__call__(Ms))
Eqsexact = np.asarray(np.vectorize(Eqmeanexact).__call__(Ms))
print(Eqsexact[0]*fEqmean)
print(Eqsexact[0]*fhadronmean)
#plt.semilogx(Ms,Eqs)
plt.semilogx(Ms,Eqsexact*fEqmean)
plt.semilogx(Ms,Eqsexact*fhadronmean)
plt.legend(['quark mean 4d', 'hadron mean 4d'])
plt.xlabel('$M_i$[g]')
plt.ylabel('$<E_q>/T_i$')
plt.ylim(6,10)
#plt.savefig('Eqmean_4d.pdf', format = 'pdf')
plt.show()
...
MBH = 1.e14
MBHoft = PBH_mass_at_t_ode(MBH, t_end)
print(MBHoft)
...
Ms = np.logspace(-5, 26, 100)
t_sol = []
for M in Ms:
	MBHoft = PBH_mass_at_t_ode(M, t_end)
	ts = MBHoft[:,0]
	ts = ts[MBHoft[:,1]>0]
	t_sol.append([M, ts[-1]])
t_sol = np.array(t_sol)
np.savetxt('BHlifetime_4d.txt', t_sol)