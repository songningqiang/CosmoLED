...
n = 4 #number of extra dimensions
...
Eprimary = np.logspace(np.log10(5.),4.,Ngrid) #Cirelli table only allows particle energy between 5GeV and 100TeV
logEprimary = np.log10(Eprimary)

T_BH = np.logspace(-9,4,Ngrid)
energy = np.logspace(-9,4,Ngrid)
...
#add contribution from tau decay
...
#primaries except for light quarks
...
spec_el += el_from_primary + el_from_decay
spec_ph += ph_from_primary + ph_from_decay
spec_nu += nu_from_primary + nu_from_decay



np.savetxt(str(n)+'d/axes.txt',(np.array([logEnergies,np.log10(T_BH)])).T,fmt='%.5e')
with open(str(n)+'d/axes.txt', 'r+') as file:
	readcontent = file.read()
	file.seek(0, 0)
	file.write('#log[10,Energy] log[10,T_BH] Data files: dN/dlog[10,Energy]dt\n')
	file.write(readcontent)
np.savetxt(str(n)+'d/positron.txt',spec_el,fmt='%.5e')
np.savetxt(str(n)+'d/gamma.txt',spec_ph,fmt='%.5e')
np.savetxt(str(n)+'d/neutrino.txt',spec_nu,fmt='%.5e')
np.savetxt(str(n)+'d/antiproton.txt',pr_from_primary,fmt='%.5e')
np.savetxt(str(n)+'d/antideutron.txt',de_from_primary,fmt='%.5e')




"""
for idy in range(len(M_BH)):
	el_from_primary[idy] += np.trapz(temp_el*(spec[:,idy])[:,None], logEprimary, axis = 0)
	ph_from_primary[idy] += np.trapz(temp_ph*(spec[:,idy])[:,None], logEprimary, axis = 0)
	nu_from_primary[idy] += np.trapz(temp_nu*(spec[:,idy])[:,None], logEprimary, axis = 0)
	pr_from_primary[idy] += np.trapz(temp_pr*(spec[:,idy])[:,None], logEprimary, axis = 0)
	de_from_primary[idy] += np.trapz(temp_de*(spec[:,idy])[:,None], logEprimary, axis = 0)
"""