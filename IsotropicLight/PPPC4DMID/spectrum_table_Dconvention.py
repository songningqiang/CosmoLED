...
##the energy unit is GeV
##n = 2 #number of extra dimensions
...
#Ngrid = 200 #number of energy and T_BH bins
...
#energy = np.logspace(-9,5.,Ngrid) #kinetic energy of secondary particles
#logEnergies = np.log10(energy)
#me = particle_dict.get('electron')[0]
#Ee = energy + me
...
#spec_el = np.asarray(np.vectorize(PBH_primary_spectrum, excluded = ['ptype','n','Mstar']).__call__(Ee[:,None], M_BH[None,:], 'electron', n, Mstar))
...
##print(M_BH[85])
##print(spec_ph[:,85]/hbar)
##plt.loglog(energy, spec_ph[:,85]/hbar,'-b')
##plt.show()
##sys.exit(0)

##keep only <5GeV part, rest obtained from Cirelli table
#spec_el[energy+me>5,:] = 0.
#spec_ph[energy>5,:] = 0.
#spec_nu[energy>5,:] = 0.
...
#"""
##muon and pion decay, not longer used
...
#for ptype in primary_list:
#	spec = np.asarray(np.vectorize(PBH_primary_spectrum, excluded = ['ptype','n','Mstar']).__call__(Edecay[:,None], M_BH[None,:], ptype, n, Mstar))
#	spec = spec*np.log(10)*Edecay[:,None]
#	temp_el = np.zeros((len(Edecay),len(energy)))
#	temp_ph = np.zeros((len(Edecay),len(energy)))
#	for idx, E in enumerate(Edecay):
#		ret_spectra = secondaries_from_decay(logEnergies,E,ptype)
#		temp_el[idx,:] = ret_spectra[0,:]
#		temp_ph[idx,:] = ret_spectra[1,:]
#	el_from_primary += np.trapz((temp_el.T)[:,:,None]*spec[None,:,:], logEdecay, axis = 1)
...
#	file.seek(0, 0)
#	file.write('#log[10, Kinetic Energy] log[10,T_BH] Data files: dN/dlog[10, Kinetic Energy]dt\n')
